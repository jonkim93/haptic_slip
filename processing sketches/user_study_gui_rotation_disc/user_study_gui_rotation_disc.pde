...
void getCommand(){
  double d = ((double) angle)*6.0/90.0*10.0/numIncrements; // SCALE FROM ANGLE TO DISTANCE; 90 deg --> 3mm displacement on all belts * 10 for conversion to increments
  println(angle);
  prevCommand = (int) d;
  println(prevCommand);
  SENT = true;
